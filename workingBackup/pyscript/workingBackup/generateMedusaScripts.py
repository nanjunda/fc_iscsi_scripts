...
def  resultOutputFile(q, r, w, b) :
    ...
    ts = os.getenv("TIME_STAMP", default=None)
    hn = os.getenv('COMPUTERNAME')
    
    csvInFile = hn + ".csv"
    logInFile = hn + ".log"
    prfInFile = hn + ".prf"   
    output_file_name_prefix = ts + "_Q" + q + "_R" + r + "_W" + w + "_B" + b + "_X" + randomPercent + "_F" + seqPercent + "_D" + runTime + "_" + protocol + "_" + phase + "_" + testType
    ...
    outFiles = [cmdOutFile, csvMv, logMv, prfMv]
    return (outFiles)               
...
def generate_command_line(q, r, w, b) :
    ...
    command_line_middle =  " -t" + threads + " -b" + bs + " -%100:r" + r + ",w" + w + ",x" + randomPercent + ",f" + seqPercent + " -Q" + q + " -d" + runTime + " -f" + targets
    cmdL = command_line_start + command_line_middle + command_line_end + outFiles[0]
    cl = [cmdL, outFiles[1], outFiles[2], outFiles[3]]
    return (cl)
...
# argument 3 - testType: normal, congestion, etc.
